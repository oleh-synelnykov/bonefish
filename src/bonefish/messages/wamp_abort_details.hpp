...
#ifndef BONEFISH_MESSAGES_WAMP_ABORT_MESSAGE_DETAILS_HPP
#define BONEFISH_MESSAGES_WAMP_ABORT_MESSAGE_DETAILS_HPP
...
class wamp_abort_details
{
public:
    wamp_abort_details();
    virtual ~wamp_abort_details();
...
inline wamp_abort_details::wamp_abort_details()
    : m_zone()
    , m_details()
{
}
...
template <typename T>
T wamp_abort_details::get_detail(const std::string& name) const
{
...
template <typename T>
T wamp_abort_details::get_detail_or(const std::string& name, const T& default_value) const
{
    const auto detail_itr = m_details.find(name);
    if (detail_itr == m_details.end()) {
...
template <typename T>
void wamp_abort_details::set_detail(const std::string& name, const T& value)
{
...
#endif // BONEFISH_MESSAGES_WAMP_ABORT_MESSAGE_DETAILS_HPP
