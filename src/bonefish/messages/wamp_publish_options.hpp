...
template <typename T>
T wamp_publish_options::get_option_or(const std::string& name, const T& default_value) const
{
    const auto option_itr = m_options.find(name);
    if (option_itr == m_options.end()) {
...
template <typename T>
void wamp_publish_options::set_option(const std::string& name, const T& value)
{
